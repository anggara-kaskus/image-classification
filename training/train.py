...
train_data_directory=os.path.join(script_directory, 'data', 'split', 'training_data')
model_directory=os.path.join(os.path.dirname(script_directory), 'models')
labels_file=os.path.join(model_directory, 'labels.txt')
...
#tf.config.gpu.set_per_process_memory_growth(False)
	...
	print("Done training in {:.3f} seconds".format(time() - t0))
	...
	if saved_model_dir:
		...
		tf.keras.utils.plot_model(
			model,
			to_file=os.path.join(saved_model_dir, 'model.png'),
			show_shapes=True,
